    ...
    protected void exec(String sinerFile, String docxTpl, String imgDir, String imgExt, String outFile,String out) throws IOException {
        ...
        if(viewGroups == null || viewGroups.size() == 0){
            ...
            for(Diagram diagram : diagrams){
                PictureRenderData renderData = createPictureRenderData(imgDir,imgExt,diagram.getDefKey());
                if(renderData != null){
...
//        System.out.println(JSONKit.toJsonString(project,true));
    ...
    protected PictureRenderData createPictureRenderData(String imgDir,String imgExt,String diagramDefKey){
        String filePath = imgDir+"/"+diagramDefKey+".png";
    ...
    private void fillModule(Module module,
        ...
        if(refDiagrams != null && refDiagrams.size() > 0){
            for(String diagramDefKey : refDiagrams){
                Diagram diagram = lookupDiagram(diagrams,diagramDefKey);
                if(diagram != null){
                    module.getDiagrams().add(diagram);
                    findDiagrams.add(diagram);
                    PictureRenderData renderData = createPictureRenderData(imgDir,imgExt,diagramDefKey);
                    if(renderData != null){
                        diagram.setRenderData(renderData);
                    }
                }
    ...
    private TableEntity lookupTableEntity(List<TableEntity> entities,String entityDefKey){
        for(TableEntity entity : entities){
            if(entityDefKey.equalsIgnoreCase(entity.getDefKey())){
                return entity;
            }
        }
        return null;
    ...
    private Diagram lookupDiagram(List<Diagram> diagrams,String diagramDefKey){
        for(Diagram diagram : diagrams){
            if(diagramDefKey.equalsIgnoreCase(diagram.getDefKey())){
                return diagram;
            }
        }
        return null;
    ...
    private Dict lookupDict(List<Dict> dicts,String dictDefKey){
        for(Dict dict : dicts){
            if(dictDefKey.equalsIgnoreCase(dict.getDefKey())){
                return dict;
            }
        }
        return null;
